...
class PlayerSscHorizontalHD (
    ...
    private val expandElapsed = FloatArray(10) { expandDuration }

    private fun startExpand(position: Int) {
        if (position !in 2..7) return
        ...
        for (position in 2..7) {
            if (expandElapsed[position] < expandDuration) {
                expandElapsed[position] =
                    (expandElapsed[position] + delta).coerceAtMost(expandDuration)
            }
        }
    }
    ...
    private fun getReceptorX(position: Int): Float {
        return getLaneX(position) - posX + luaRecepts.screenX
    }
    ...
    private fun drawExpandEffect(position: Int) {
        ...
        val logicalY = topPos
        val drawWidth = metrics.drawWidth(sizeScale)
        val drawHeight = metrics.drawHeight(sizeScale)
        val drawX = metrics.drawX(logicalX, sizeScale)
        val drawY = metrics.drawY(logicalY, sizeScale)

        val receptorCenterX = getLaneX(position) + (medidaFlechasHorizontal * 0.5f) + luaRecepts.screenX
        val receptorCenterY = medidaFlechasHorizontal * 1.5f
        val originX = receptorCenterX - drawX
        val originY = receptorCenterY - drawY
        ...
        batch.draw(receptor, drawX, drawY, originX, originY, drawWidth, drawHeight, scale, scale, 0f)
    ...
    private fun drawPressedPad(position: Int) {
        ...
        if (showPadB == 3) {
            batch.draw(
                screen.arrayPad4[position % 5],
                padPositionsHorizontalHD[position][0],
                padPositionsHorizontalHD[position][1],
                widthBtnsHorizontal,
    ...
    private fun drawGauge(gauge: Float, stretchProgress: Float) {
        ...
        val visibleWidth = if (safeGauge >= 1f) screen.maxWidth else (baseVisibleWidth + stretchWidth).coerceAtMost(screen.maxWidth)
        ...
        if (safeGauge < 0.98f && safeGauge > 0f) {
            val tipX = initGaujePosition + visibleWidth
    ...
    private val lightningX = initGaujePosition - (screen.maxWidth * 0.1f)
    private val lightningY = screen.posYGauje - ((lightningExtraHeight - screen.maxlHeight) / 2f)
